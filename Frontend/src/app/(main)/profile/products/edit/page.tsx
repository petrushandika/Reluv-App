...
import { Product, Variant, Condition } from "@/features/(main)/products/types";
import { toast } from "sonner";
import Image from "next/image";
import CustomSelect from "@/shared/components/common/CustomSelect";
import CategorySelector from "@/shared/components/common/CategorySelector";
import { uploadImage } from "@/features/(main)/sell/api/sellApi";
  ...
  const handleSaveProduct = async () => {
    ...
    if (!productValidation.success) {
      ...
      if (
        ...
        productValidation.error.errors[0]
      ) {
                      ...
                      {formData.images.length < maxTotalImages && (
                        <>
                          ...
                          >
                            ...
                            <p className="mt-1 text-[10px] sm:text-xs text-center">
                              Add more
                            </p>
                          </div>
                        </>
                  ...
                  <div className="grid grid-cols-1 md:grid-cols-2 gap-3 sm:gap-4">
                    ...
                    <CustomSelect
                      ...
                      value={
                        variants[0].color &&
                        colorOptions.find(
                          (opt) => opt.value === variants[0].color
                        )
                          ? variants[0].color
                          : variants[0].color
                          ? "OTHER"
                  ...
                  {!fieldErrors.description &&
                    formData.description.length > 9500 && (
                      <p className="text-[10px] sm:text-xs text-amber-600 dark:text-amber-400 mt-1">
                        {10000 - formData.description.length} characters
                        remaining
                      </p>
                    )}
                </div>
